...
import com.detonomics.budgettuner.backend.mainapplicationfeatures.BudgetManager;
import com.detonomics.budgettuner.backend.mainapplicationfeatures.BudgetTotals;
import com.detonomics.budgettuner.backend.mainapplicationfeatures.BudgetYear;
import com.detonomics.budgettuner.backend.mainapplicationfeatures.ExpenseCategory;
import com.detonomics.budgettuner.backend.mainapplicationfeatures.Ministry;
import com.detonomics.budgettuner.backend.mainapplicationfeatures.RevenueCategory;
import com.detonomics.budgettuner.backend.mainapplicationfeatures.Summary;
...
public class BudgetDetailsController {
    ...
    @FXML private Label revenuesValue;
    @FXML private Label expensesValue;
    @FXML private Label resultValue;

    @FXML private LineChart<String, Number> totalsChart;
    @FXML private NumberAxis amountAxis;

    @FXML private LineChart<String, Number> resultChart;
    @FXML private NumberAxis resultAmountAxis;

    @FXML private VBox topRevenuesBox;
    @FXML private VBox topExpensesBox;
    @FXML private VBox topMinistriesBox;

    @FXML private Button menuButton;
    @FXML private Pane menuOverlay;
    @FXML private VBox menuDrawer;
    ...
    private void loadPreviousBudgetIfAvailable() {
        ...
        BudgetManager manager = new BudgetManager(dbPath);
        List<BudgetTotals> totals = manager.loadAllBudgetTotals();
        ...
        try {
            previousBudget = manager.loadBudgetYearByYear(previousYear);
        ...
        for (RevenueCategory r : previousBudget.getRevenues()) {
            prevRevenueAmountByKey.put(key(r.getCode(), r.getName()), r.getAmount());
        }
        for (ExpenseCategory e : previousBudget.getExpenses()) {
            prevExpenseAmountByKey.put(key(e.getCode(), e.getName()), e.getAmount());
        }
        for (Ministry m : previousBudget.getMinistries()) {
            prevMinistryAmountByKey.put(key(m.getCode(), m.getName()), m.getTotalBudget());
        }
    }
    ...
    private void render() {
        ...
        if (summary != null) {
            ...
            expensesValue.setText(formatMoney(summary.getTotalExpenses()));
            resultValue.setText(formatMoney(summary.getBudgetResult()));

            resultValue.getStyleClass().removeAll("metric-positive", "metric-negative");
            if (summary.getBudgetResult() >= 0) {
    ...
    private void renderTopMinistries() {
        ...
        for (Ministry m : top) {
            ...
            topMinistriesBox.getChildren().add(dataRow(String.valueOf(m.getCode()), m.getName(), prev, m.getTotalBudget()));
        }
    }
    ...
    private static String formatMoney(double value) {
        NumberFormat nf = NumberFormat.getNumberInstance(new Locale("el", "GR"));
